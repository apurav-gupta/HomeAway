...
import { travelerLogin } from "../../queries/queries";
  ...
  doGraphLogin = e => {
    ...
    this.props.client
      ...
      .then(response => {
        ...
        if (response.data.travelerLogin.status === 200) {
          localStorage.setItem(
            "travellerToken",
            response.data.travelerLogin.token
          );
          this.props.history.push("/home");
        } else if (response.data.travelerLogin.status === 400) {
          alert("User Doesn't exist, please sign up before logging in !!");
          // this.props.history.push('/home');
        } else if (response.data.travelerLogin.status === 404) {
