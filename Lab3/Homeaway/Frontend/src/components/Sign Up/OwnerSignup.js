...
import { ownerSignUpMutation } from "../../mutation/mutation";
  ...
  doGraphSignUp = event => {
    ...
    this.props
      ...
      .then(response => {
        ...
        // console.log("status is - " + response.data.ownerSignUp.status);
        // console.log("email is is - " + response.data.ownerSignUp.email);

        if (response.data.ownerSignUp.status === 200) {
          localStorage.setItem("ownerToken", response.data.ownerSignUp.token);
          this.props.history.push("/Dashboard");
        } else if (response.data.ownerSignUp.status === 204) {
          alert("Owner already exists, please login !!");
